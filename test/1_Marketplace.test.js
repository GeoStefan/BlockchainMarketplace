...
contract("Marketplace", function (accounts) {
    ...
    it("creates new user correctly", async () => {
        const charge = 2, time = 23, type = 1, name = "Ion", category = "Math", userTokens = 3, userAddress = accounts[1];

        let result = await this.marketplace.createActor(charge, time, type, name, category, userTokens, userAddress, { from: accounts[0] });
        ...
        assert.equal(actor.time.toNumber(), time, "Time wasn't correct");
